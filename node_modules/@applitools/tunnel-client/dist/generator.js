...
async function* makeGenerator({ settings, logger, }) {
    ...
    const req = (0, req_1.makeReq)({
        ...
        retry: {
            timeout: 10000,
            validate: ({ response, error }) => (response === null || response === void 0 ? void 0 : response.status) !== 200 &&
        ...
        connectionTimeout: (_b = settings.timeout) !== null && _b !== void 0 ? _b : 5 * 60000,
    });
    logger.info('Initializing tunnel agent with settings', {
        pollingServerUrl: settings.pollingServerUrl,
        pollingTimeout: settings.pollingTimeout,
        agentId: settings.agentId,
        timeout: settings.timeout,
        envInfo: settings.envInfo,
    ...
    const response = await req(`/tunnel-agents/${settings.agentId}/init`, {
        ...
        logger.error('Failed to initialize tunnel agent', error);
        throw new Error(`Failed to initialize tunnel agent: ${error.message}`);
    });
    const instanceId = await response.json().then((result) => result.instance_id);
    logger.log('Tunnel agent initialized with id:', instanceId);
    let pendingRequestMessages = [];
    let outgoingResponseMessages = [];
    let tunnelsEventMessage;
    let metricsEventMessage;
    let ecHealthCheckStatus = { status: 'unknown', verified_at: Date.now() };
    while (true) {
        const response = await req(`/tunnel-agents/${settings.agentId}/agentpoll`, {
            ...
            body: {
                ...
                ec_healthcheck: ecHealthCheckStatus,
                agent_status: 'OK',
            },
        }).catch(error => {
            logger.error(`Failed to poll tunnel agent: ${error.message}`);
            // the process will be restarted by the primary process
            throw new Error(`Failed to poll tunnel agent: ${error.message}`);
        });
        ...
        try {
            ...
            incomingRequestMessages = result.tasks.flatMap((task) => {
                if (task.type === 'CREATE_TUNNEL') {
                    logger.mask(task.apiKey);
                    logger.mask(task.eyesServerUrl);
                    return {
        ...
        logger.log('Outgoing messages:', outgoingMessages);
        outgoingMessages.forEach(message => {
            ...
            else if (message.name === 'TunnelClient.EcHealthCheck')
                ecHealthCheckStatus = message.payload;
            else
