...
function makeImage(data, defaultLogger) {
    let image, size;
    let transforms = { rotate: 0, scale: 1, crop: undefined, modifiers: [] };
    const logger = defaultLogger === null || defaultLogger === void 0 ? void 0 : defaultLogger.extend({ label: 'image' });
    if (utils.types.isUint8Array(data) || utils.types.isAnyArrayBuffer(data)) {
        ...
        if ((0, png_1.isPngBuffer)(buffer)) {
            image = (0, png_1.fromPngBuffer)(buffer, logger);
            size = (0, png_1.extractPngSize)(buffer);
        }
        else if ((0, jpeg_1.isJpegBuffer)(buffer)) {
            image = (0, jpeg_1.fromJpegBuffer)(buffer);
            size = (0, jpeg_1.extractJpegSize)(buffer);
        }
        else if ((0, bmp_1.isBmpBuffer)(buffer)) {
            image = (0, bmp_1.fromBmpBuffer)(buffer);
            size = (0, bmp_1.extractBmpSize)(buffer);
        }
        else if ((0, gif_1.isGifBuffer)(buffer)) {
            image = (0, gif_1.fromGifBuffer)(buffer);
            size = (0, gif_1.extractGifSize)(buffer);
        }
        else {
    ...
    return {
        ...
        async toPng() {
            return (0, png_1.toPng)(await this.toObject(), logger);
        },
        async toFile(path) {
            return toFile(await image, path, logger);
        },
        async toObject() {
        ...
        async debug(debug) {
            ...
            return toFile(transformedImage, path_1.default.join(debug.path, filename), logger).catch(() => undefined);
        },
    };
}
...
async function toFile(image, filepath, logger) {
    const buffer = await (0, png_1.toPng)(image, logger);
