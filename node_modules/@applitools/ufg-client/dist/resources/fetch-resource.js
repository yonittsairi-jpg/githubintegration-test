...
const create_authorization_header_1 = require("../utils/create-authorization-header");
...
function makeFetchResource({ accessToken, eyesServerUrl, apiKey, tunnelIds, concurrency = 100, retryLimit = 5, timeout = 30 * 1000, streamingTimeout = 30 * 1000, cache = new Map(), fetch, logger: mainLogger, }) {
    ...
    return concurrency ? (0, throat_1.default)(concurrency, fetchResource) : fetchResource;
    async function fetchResource({ resource, settings = {}, logger = mainLogger, }) {
        ...
        runningRequest = req(resource.url, {
            headers: {
                ...
                Authorization: (_d = (_c = settings.headers) === null || _c === void 0 ? void 0 : _c.Authorization) !== null && _d !== void 0 ? _d : (0, create_authorization_header_1.createAuthorizationHeader)({ referrerUrl: settings.url, resourceUrl: resource.url }),
            },
            proxy: resourceUrl => {
...
function handleTunneledResource({ eyesServerUrl, apiKey, accessToken, tunnelIds, }) {
    if (!tunnelIds)
        return {};
    return {
...
function handleLogs({ logger }) {
    return {
        async beforeRequest({ request }) {
            logger === null || logger === void 0 ? void 0 : logger.log(`Resource with url ${request.url} will be fetched using headers`, Object.fromEntries(request.headers.entries()), `and body ${JSON.stringify(await request.clone().text())}`);
        },
