...
function makeUFGRequests({ settings, logger }) {
    ...
    const req = (0, req_ufg_1.makeReqUFG)({ settings, logger: mainLogger });
    ...
    async function startRenders({ renders, logger = mainLogger, }) {
        ...
        const rendersOptions = renders.map(({ target, settings }) => {
            var _a;
            const renderOptions = {
                url: target.source,
        ...
        const chunksResponse = await Promise.all(chunks.map(async (chunk) => {
            const response = await callRenderThrottled(chunk);
    ...
    async function checkResources({ resources, logger = mainLogger, }) {
        ...
        if (results.length !== resources.length) {
            throw new Error(`Unexpected number of presented resources - ${results.length} instead of ${resources.length}`);
        }
        if (results.some(result => result === null)) {
            throw new Error('Unexpected null value in presented resources');
        }
    ...
    async function getActualEnvironments({ settings, logger = mainLogger, }) {
        ...
        const results = await utils.general.dedupAndMap(settings, async (settings) => {
