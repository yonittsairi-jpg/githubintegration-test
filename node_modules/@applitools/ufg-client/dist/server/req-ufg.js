...
function makeReqUFG({ settings, logger }) {
    ...
    return (0, req_1.makeReq)({
        ...
        retry: {
            limit: 5,
            timeout: new Array(5).fill(0).map((_, i) => Math.pow(2.317, i) * 200),
    ...
    return {
        async afterResponse({ request, response, options }) {
            if ((options === null || options === void 0 ? void 0 : options.expected) &&
                ...
                throw new Error(`Request "${options === null || options === void 0 ? void 0 : options.name}" that was sent to the address "[${request.method}]${request.url}" failed due to unexpected status ${response.statusText}(${response.status}) with body ${JSON.stringify(await response.clone().text())}`);
            }
        },
    };
}
